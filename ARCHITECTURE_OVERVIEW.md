...
- **Method:** Multi-model parallel inference (9 specialist models)
- **Used in:** `src/services/apiService.ts` via Supabase Edge Function `parallel-leaf-scan`

### 2. **Supabase** (Backend & ML Orchestration)
- **Purpose:** Database, Authentication, and Edge Functions
- **Database:** Postgres (Profiles, Recipes, Comments, Likes)
- **Edge Functions:** `parallel-leaf-scan` (orchestrates 9-model parallel inference)
- **Used in:** `src/lib/supabaseClient.ts`, `src/services/parallelScanService.ts`

### 3. **OpenRouter API** (AI Chat)
- **Purpose:** LLM-powered chat functionality
- **Endpoint:** `https://openrouter.ai/api/v1/chat/completions`
- **Model:** `meta-llama/llama-3.1-8b-instruct`
- **Used in:** `src/services/apiService.ts`

### 4. **N8N Webhook** (Alternative Chat - Optional)
- **Purpose:** Alternative chat backend via workflow automation
- **Configurable:** Via settings
- **Used in:** `src/services/apiService.ts`

### 5. **Database (Supabase Postgres)** ✅
- ✅ All user data (Profiles, Conversations, Likes, Comments)
- ✅ Traditional Moroccan recipes (community contributed)
