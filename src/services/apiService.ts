...
import { ParallelScanService, ParallelScanResponse } from '@/services/parallelScanService';
...
export class APIService {
  ...
  static async detectLeaf(imageBase64: string): Promise<RoboflowResponse> {
    try {
      ...
      const endpoint = import.meta.env.VITE_ROBOFLOW_ENDPOINT || "https://serverless.roboflow.com/vegetable-detection-rtnua-ymjxz/1";
      if (!apiKey) {
  ...
  /**
   * Parallel 9-model leaf detection via Supabase edge function.
   * Falls back to single-model detectLeaf() if edge function fails
   * and parallelScanFallback is enabled in settings.
   */
  static async detectLeafParallel(imageBase64: string): Promise<ParallelScanResponse> {
    try {
      const response = await ParallelScanService.scanAllModels(imageBase64);

      // Track the top result in analytics
      if (response.ranked.length > 0 && response.ranked[0].confidence > 0) {
        AnalyticsService.recordScan(response.ranked[0].leafName);
      } else {
        AnalyticsService.recordScan();
      }
      ...
      logger.error('Parallel scan failed:', error);

      const settings = SettingsService.getSettings();
      if (settings.parallelScanFallback) {
        logger.debug('Falling back to single-model detectLeaf');
        const fallbackResult = await this.detectLeaf(imageBase64);

        // Convert single-model result to ParallelScanResponse shape
        const topPrediction = fallbackResult.predictions[0];
        return {
          ranked: topPrediction ? [{
            leafId: 0,
            leafName: topPrediction.class,
            leafNameFr: topPrediction.class,
            confidence: topPrediction.confidence,
            predictionCount: fallbackResult.predictions.length,
            avgConfidence: fallbackResult.predictions.reduce((s, p) => s + p.confidence, 0) / fallbackResult.predictions.length,
            tieBreakScore: 0,
            modelVersion: 1,
  ...
  static async sendChatMessage(messages: ChatMessage[]): Promise<string> {
    try {
      ...
      if (error) {
        logger.warn('Edge Function error, falling back to direct OpenRouter:', error);
        return this.sendChatMessageToOpenRouter(messages);
      }

      if (data?.error) {
        logger.warn('Edge Function returned error, falling back:', data.error);
        return this.sendChatMessageToOpenRouter(messages);
      }
