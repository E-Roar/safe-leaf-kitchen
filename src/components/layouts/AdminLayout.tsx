...
export const AdminLayout = () => {
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex">
            {/* Sidebar Desktop */}
            <aside className="hidden lg:flex w-64 flex-col bg-slate-900 text-slate-300 border-r border-slate-800 fixed inset-y-0 left-0 z-50">
                ...
                <nav className="flex-1 p-4 space-y-1">
                    ...
                    <div className="pt-4 mt-4 border-t border-slate-800">
                        <NavLink
                            to="/"
                            className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all duration-200 hover:bg-slate-800 hover:text-white text-slate-400"
                        >
            ...
            {isSidebarOpen && (
                <div className="fixed inset-0 z-30 lg:hidden">
                    ...
                    <nav className="absolute left-0 top-0 bottom-0 w-64 bg-slate-900 text-slate-300 p-4 pt-20 space-y-2 border-r border-slate-800">
                        {navItems.map((item) => (
                            <NavLink
                                key={item.href}
                                to={item.href}
                        ...
                        <NavLink
                            to="/"
                            onClick={() => setIsSidebarOpen(false)}
                            className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-slate-400 hover:bg-slate-800 w-full mt-4"
                        >
                            <ArrowLeft className="w-5 h-5" />
                            Back to App
                        </NavLink>
                        <button
                            onClick={handleSignOut}
                            className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium text-red-400 hover:bg-slate-800 w-full mt-2"
                        >
