...
interface LeafGalleryProps {
  leafId: number;
  leafName: string;
  galleryImages?: string[]; // Images from DB
  className?: string;
}
...
export function LeafGallery({ leafId, leafName, galleryImages: _galleryImages = [], className }: LeafGalleryProps) {
  const { t } = useI18n();
  ...
  logger.debug(`🖼️ LeafGallery rendered for:`, { leafId, leafName });
  ...
  useEffect(() => {
    const generateGalleryImages = async () => {
      ...
      logger.debug(`🍃 LeafGallery Debug:`, {
        leafName,
        folderName,
      ...
      possibleImageNames.forEach((imageName, index) => {
        ...
        logger.debug(`🖼️ Adding image path: ${imagePath}`);
        galleryImages.push({
          src: imagePath,
      ...
      setTimeout(() => {
        logger.debug('⏰ Leaf fallback timeout triggered - forcing loading complete');
        setIsLoading(false);
      }, 3000);
    };
  ...
  useEffect(() => {
    if (images.length > 0) {
      logger.debug(`🔄 Starting to load ${images.length} images`);
  ...
  const handleImageLoad = (index: number, event: React.SyntheticEvent<HTMLImageElement>) => {
    ...
    logger.debug(`✅ Leaf image loaded successfully: ${img.src}`);
  };
  ...
  const handleImageError = (index: number, src: string) => {
    logger.debug(`❌ Image failed to load: ${src}`);
