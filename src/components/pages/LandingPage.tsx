...
import { Leaf, Scan, ChefHat, MessageCircle, Users, Globe2, ShieldCheck, Building2, ArrowRight, Sun, Moon, Sparkles, Heart, TrendingUp, CheckCircle, Star, BookOpen, RefreshCw } from "lucide-react";
...
export default function LandingPage(_props: LandingPageProps) {
  ...
  const onNavigateToChat = () => navigate('/chat');
  const onNavigateToDemo = () => {
    safeStorage.set('demoMode', 'true');
    navigate('/chat');
  };
  const onNavigateToRecipes = () => navigate('/recipes');
  const onNavigateToScan = () => {
    navigate('/chat');
    setTimeout(() => window.dispatchEvent(new Event('openCameraScan')), 0);
  };
  const onNavigateToLeaves = () => navigate('/leaves');
  const onToggleTheme = () => {
    setTheme(prev => {
            ...
            <div className="max-w-4xl mx-auto text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-4 mb-8 justify-center">
                <button
                  ...
                  className="btn-organic px-8 py-4 text-lg font-semibold text-primary-foreground flex items-center gap-3 justify-center hover:scale-105 transition-all duration-300 shadow-glow group ring-2 ring-amber-400/50 bg-gradient-to-r from-amber-500 to-orange-500"
                >
