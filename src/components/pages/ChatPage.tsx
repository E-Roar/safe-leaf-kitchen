...
- Info response: "${leafType} leaves are rich in antioxidants and vitamins. They provide excellent nutritional benefits."`
        ...
        const matched = leavesData
          .map(l => ({
            id: l.id,
            name: l.name.en,
            tokens: [l.name.en, l.name.fr, ...l.aliases].map(a => a.toLowerCase())
          }))
          .find(entry => entry.tokens.some(t => t === normalized || normalized.includes(t) || t.includes(normalized)));
        if (matched) {
