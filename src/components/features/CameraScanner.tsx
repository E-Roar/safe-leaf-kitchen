...
interface CameraScannerProps {
  onClose: () => void;
  onDetection: (result: ParallelScanResponse | DetectionResult[]) => void;
}
...
export default function CameraScanner({ onClose, onDetection }: CameraScannerProps) {
  ...
  const captureAndAnalyze = async () => {
    ...
    try {
      ...
      // Send to Roboflow API — parallel or single
      const settings = SettingsService.getSettings();

      if (settings.parallelScanEnabled) {
        // Parallel 9-model scan
        logger.debug("Using parallel 9-model scan");
        const parallelResult = await APIService.detectLeafParallel(base64Image);
        logger.debug("Parallel scan response:", parallelResult);
        onDetection(parallelResult);
        setTimeout(() => { onClose(); }, 500);
      } else {
        // Legacy single-model scan
      ...
      <div className="p-4 sm:p-6 text-center text-white/80 bg-black/50 backdrop-blur-sm" style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}>
        <p className="text-sm sm:text-base">
          {isCameraLoading
            ? "Initializing camera..."
            : isScanning
              ? "Querying 9 models simultaneously..."
