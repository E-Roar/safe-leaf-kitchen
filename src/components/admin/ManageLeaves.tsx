...
export const ManageLeaves = () => {
    ...
    return (
        <div className="space-y-6">
            ...
            {editingLeaf && (
                <Dialog open={true} onOpenChange={(open) => { if (!open) setEditingLeaf(null); }}>
                <DialogContent className="sm:max-w-lg max-h-[85vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>{editingLeaf.id ? 'Edit Leaf' : 'New Leaf'}</DialogTitle>
                        <DialogDescription>Structured data here also trains the AI.</DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4 py-2">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <Label>Scientific Name (Latin)</Label>
                                <Input
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-3 mt-1">
                                <div>
                                    <Label className="text-xs text-muted-foreground">Proteins (%)</Label>
                                    <Input type="number" step="0.01" value={editingLeaf.highlights?.proteins_percent ?? ''} onChange={e => setEditingLeaf(prev => ({ ...prev, highlights: { ...prev?.highlights, proteins_percent: parseFloat(e.target.value) || 0 } }))} />
                                </div>
                                <div>
                                    <Label className="text-xs text-muted-foreground">Calcium (mg/100g)</Label>
                                    <Input type="number" step="0.01" value={editingLeaf.highlights?.calcium_mg_per_100g ?? ''} onChange={e => setEditingLeaf(prev => ({ ...prev, highlights: { ...prev?.highlights, calcium_mg_per_100g: parseFloat(e.target.value) || 0 } }))} />
                                </div>
                                <div>
                                    <Label className="text-xs text-muted-foreground">Flavonoids (mg/100g)</Label>
                                    <Input type="number" step="0.01" value={editingLeaf.highlights?.flavonoids_mg_per_100g ?? ''} onChange={e => setEditingLeaf(prev => ({ ...prev, highlights: { ...prev?.highlights, flavonoids_mg_per_100g: parseFloat(e.target.value) || 0 } }))} />
                                </div>
                                <div>
                                    <Label className="text-xs text-muted-foreground">Polyphenols (mg/100g)</Label>
                                    <Input type="number" step="0.01" value={editingLeaf.highlights?.polyphenols_mg_per_100g ?? ''} onChange={e => setEditingLeaf(prev => ({ ...prev, highlights: { ...prev?.highlights, polyphenols_mg_per_100g: parseFloat(e.target.value) || 0 } }))} />
                                </div>
                                <div>
                                    <Label className="text-xs text-muted-foreground">Antioxidant Classification</Label>
                                    <Input value={editingLeaf.highlights?.antioxidant_classification || ''} onChange={e => setEditingLeaf(prev => ({ ...prev, highlights: { ...prev?.highlights, antioxidant_classification: e.target.value } }))} />
                                </div>
                            </div>
                        </div>
